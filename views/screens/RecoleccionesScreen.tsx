...
type RecoleccionesScreenNavigationProp = StackNavigationProp<RootStackParamList, 'Recolecciones'>;

const RecoleccionesScreen: React.FC = () => {
  const navigation = useNavigation<RecoleccionesScreenNavigationProp>();
  ...
  // Estados para alerta de confirmación de cancelación
  const [cancelAlertVisible, setCancelAlertVisible] = useState(false);
  const [recoleccionToCancel, setRecoleccionToCancel] = useState<string | null>(null);
  ...
  const loadRecolecciones = useCallback(async () => {
    try {
      ...
      // Obtener recolecciones del usuario con estado 0 (programadas)
      const recoleccionesData = await RecoleccionPresenter.getRecoleccionesByUsuarioAndEstado(user.id, 0);
  ...
  }, [searchText, startDate, endDate, recolecciones]);
  ...
  const setupWebSockets = useCallback(async () => {
    try {
      const recSocket = await initSocket();
      
      // Escuchar eventos de recolecciones
      recSocket.on('recoleccion_creada', async (nuevaRecoleccion: Recoleccion) => {
        // Solo recargar si la recolección es del usuario actual y está programada
        const user = await AuthPresenter.getCurrentUser();
        if (user && nuevaRecoleccion.id_usuario === user.id && nuevaRecoleccion.cumplimiento === 0) {
          loadRecolecciones();
        }
      });
      
      recSocket.on('recoleccion_actualizada', async (recoleccionActualizada: Recoleccion) => {
        // Solo recargar si la recolección es del usuario actual
        const user = await AuthPresenter.getCurrentUser();
        if (user && recoleccionActualizada.id_usuario === user.id) {
            loadRecolecciones();
        }
      });
      
      recSocket.on('recoleccion_cancelada', async (recoleccionCancelada: Recoleccion) => {
        // Solo recargar si la recolección es del usuario actual
        const user = await AuthPresenter.getCurrentUser();
        if (user && recoleccionCancelada.id_usuario === user.id) {
          loadRecolecciones();
        }
      });
      
      recSocket.on('error', (error: any) => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (socket) {
        socket.off('recoleccion_creada');
        socket.off('recoleccion_actualizada');
        socket.off('recoleccion_cancelada');
        socket.off('error');
      }
    };
  }, [loadRecolecciones, setupWebSockets]);
  ...
  const handleCancelAlertConfirm = () => {
    if (recoleccionToCancel) {
      confirmCancelRecoleccion();
    }
    setCancelAlertVisible(false);
    setRecoleccionToCancel(null);
  };
  ...
  const hasActiveFilters = () => {
    return searchText !== '' || startDate !== null || endDate !== null;
  };

  // Función para mostrar la alerta de cancelación
  const cancelarRecoleccion = (noRecoleccion: string) => {
    setRecoleccionToCancel(noRecoleccion);
    setCancelAlertVisible(true);
  };

  // Función para confirmar la cancelación
  const confirmCancelRecoleccion = async () => {
    ...
    try {
      ...
      // Obtener recolecciones solo cuando el usuario confirma
      const todasRecolecciones = await RecoleccionPresenter.getRecoleccionesByUsuario(user.id);
      const recoleccionesACancelar = todasRecolecciones.filter(
        (rec: Recoleccion) => rec.NoRecoleccion === recoleccionToCancel && rec.cumplimiento === 0
      );

      if (recoleccionesACancelar.length === 0) {
        showAlert('Error', 'No se encontraron recolecciones para cancelar o ya fueron canceladas', 'error');
        setLoading(false);
        return;
      }

      // Cancelar cada recolección individual
      for (const rec of recoleccionesACancelar) {
        await RecoleccionPresenter.cancelarRecoleccion(rec.id);
      }

      showAlert('Éxito', 'Recolección cancelada correctamente', 'success');
      loadRecolecciones(); // Recargar la lista
    } catch (error: any) {
      showAlert('Error', error.message || 'Error al cancelar la recolección', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderRecoleccionItem = ({ item }: { item: RecoleccionAgrupada }) => (
    <View style={styles.recoleccionItem}>
      ...
      <View style={styles.recoleccionInfo}>
        ...
        {/* Botón de cancelar para recolecciones programadas */}
        {item.cumplimiento === 0 && (
          <TouchableOpacity 
            style={styles.cancelButton}
            onPress={() => cancelarRecoleccion(item.NoRecoleccion)}
          >
            <Icon name="times-circle" size={16} color="#dc3545" />
            <Text style={styles.cancelButtonText}>Cancelar Recolección</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.titleContainer}>
          <Icon name="list-alt" size={30} color="white" style={styles.titleIcon} />
          <Text style={styles.titleText}>Mis Recolecciones</Text>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Filtrar Recolecciones</Text>
      ...
      {/* Alert Component para confirmación de cancelación */}
      <AlertComponent
        visible={cancelAlertVisible}
        title="Confirmar cancelación"
        message={`¿Estás seguro de que deseas cancelar la recolección #${recoleccionToCancel}?`}
        type="confirmation"
        confirmText="Sí, cancelar"
        cancelText="No"
        onConfirm={handleCancelAlertConfirm}
        onCancel={handleCancelAlertCancel}
        onClose={handleCancelAlertCancel}
      />
    </View>
  );
};
